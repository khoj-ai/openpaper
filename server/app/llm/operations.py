...
class Operations:
    ...
    def validate_and_extract_json(self, json_data: str) -> dict:
        ...
        if "```" in json_data:
            ...
            for block in code_blocks:
                ...
                # Remove standalone words that appear after closing braces/brackets
                block = re.sub(r"}\s+\w+\s+}", "}}", block)  # Fix case like "} word }"
                block = re.sub(r"}\s+\w+\s+,", "},", block)  # Fix case like "} word ,"
    ...
    def augment_hypothesis(
        ...
        """
        formatted_prompt = IMPROVE_QUESTION_TO_HYPOTHESIS.format(
            question=question,
        )
    ...
    def generate_steps(
        ...
        """
        Generate steps to test the hypothesis using the specified model
        """
        # Construct the expected schema for the response
        schema = HypothesisFanOut.model_json_schema()

        # Format the prompt with the hypothesis and schema
