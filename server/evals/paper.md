...
Large language models increasingly help researchers read scientific literature, but evaluating whether a model cites verifiable evidence remains a structural challenge that standard LLM-as-judge metrics fail to surface. We introduce **ResearchQA**, a 6,211-question single-paper QA benchmark across 494 open-access papers in eight domains and four question types (lookup, comprehension, multi-hop, adversarial), graded by a deterministic citation matcher alongside an anchored-rubric LLM judge. Two design decisions distinguish ResearchQA: the schema admits multiple valid alternative passages per required citation, and the adversarial slice scores grounded refusal rather than silent refusal — both invisible to embedding-similarity or aggregate judge metrics. Across eight frontier and open-weight LLMs run through a citation-grounded chat-with-paper harness ([Open Paper](https://openpaper.ai)), the deterministic citation metrics discriminate systems ~4× more sharply than the LLM-judge metrics: judge scores compress into a 0.255-point band while section coverage and citation accuracy spread by 14–32 points. Open-weight models on Cerebras reach near-frontier citation accuracy (Zai GLM 4.7: 0.840 vs the best closed score of 0.845) at 3–6× lower per-row latency. The [benchmark](https://huggingface.co/datasets/khoj-ai/ResearchQA), the [harness](https://github.com/khoj-ai/openpaper/tree/92cd89c85dbabb9bff8baa281e0678472ccb2ab4/server/evals), and the judge prompt are released openly.
...
The citation matcher decides whether a model's quoted citation appears in the paper's extracted text. Naively this is a substring check, but [PyPDF2](https://pypi.org/project/PyPDF2/)'s text extraction introduces enough noise — line-break hyphenation, mid-word spaces, ligatures, smart quotes — that faithfully-quoted citations regularly fail an exact match. The matcher absorbs this noise through a normalization pipeline (Unicode folding plus whitespace collapse) applied to both sides, with a final whitespace-stripped fallback that reclaims citations broken by mid-word PDF artifacts; full implementation details are in the released code. The impact is large: the fallbacks lifted citation accuracy from 0.554 to 0.817 on the same 100-row Gemini run with no change to model behavior — a 26-point gain that is purely about not under-counting correct citations. We chose this over embedding similarity because the failure modes diverge: a normalization-based matcher rejects fabricated content that does not appear in the paper, while an embedding matcher would pass a fabricated-but-on-topic quote that lives near real paper content in embedding space..
