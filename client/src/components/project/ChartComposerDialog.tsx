...
export function ChartComposerDialog({ open, onOpenChange, projectId, papers, onCreated }: Props) {
    ...
    /** Rename an axis. The key is the plan's identifier — `fields` entries and
     * `calculation.inputs` refer to it — so editing the label leaves it alone.
     * Re-deriving it from the label meant every keystroke renamed the field and
     * everything pointing at it had to be rewritten in lockstep. */
    const updateField = (name: "x" | "y", label: string) => {
        setGenerationError(null);
        if (!plan) return;
        const { key } = plan[name];
        setPlan({
            ...
            fields: plan.fields.map(field => (field.key === key ? { ...field, label } : field)),
        });
    };
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
            ...
            {!plan ? (
                <div className="space-y-4">
                    <div>
                        <Label htmlFor="chart-request">What should the chart show?</Label>
                        <Textarea
                            id="chart-request"
                            className="mt-2"
                            value={prompt}
                            onChange={event => setPrompt(event.target.value)}
                            placeholder="Compare sample size against accuracy across these papers"
                        />
                    </div>
                    <div>
                        <Label>Paper scope</Label>
                        <div className="mt-2 max-h-36 space-y-2 overflow-y-auto rounded border p-2">
                            {papers.map(paper => (
                                <label key={paper.id} className="flex gap-2 text-sm">
                                    <input
                                        type="checkbox"
                                        checked={paperIds.includes(paper.id)}
                                        onChange={() => setPaperIds(ids => (
                                            ids.includes(paper.id)
                                                ? ids.filter(id => id !== paper.id)
                                                : [...ids, paper.id]
                                        ))}
                                    />
                                    {paper.title}
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        <Button onClick={propose} disabled={loading || !prompt.trim() || paperIds.length === 0}>
                ...
                <div className="space-y-4">
                    <p className="rounded bg-muted p-2 text-xs text-muted-foreground">
                        Review and customize this plan. Generation will search the selected papers
                        and include only directly cited values.
                    </p>
                    <div>
                    ...
                    <div>
                        ...
                        <Select
                            value={plan.chart_type}
                            onValueChange={(value: ChartPlan["chart_type"]) => {
                                setGenerationError(null);
                                setPlan({ ...plan, chart_type: value });
                            }}
                        >
                    ...
                    {plan.calculation && (
                        <p className="text-xs text-muted-foreground">
                            Calculated y: {plan.calculation.spec}
                        </p>
                    )}
                    {generationError && (
                        <p
                            role="alert"
                            className="rounded border border-amber-300 bg-amber-50 p-3 text-xs leading-relaxed text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200"
                        >
                            {generationError}
                        </p>
                    )}
                    <div className="flex justify-between gap-2">
                        ...
                        <Button onClick={generate} disabled={loading || !plan.title.trim()}>
                            {loading
