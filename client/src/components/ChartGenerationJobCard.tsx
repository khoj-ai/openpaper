...
export function ChartGenerationJobCard({ job, onOpenPaper }: {
    ...
    return <section className="rounded-lg border bg-card p-3 text-card-foreground">
        ...
        {job.trace && !job.artifact && <div className="mt-2"><MessageTraceViewer trace={job.trace} /></div>}
        {job.artifact && <ChartArtifactCard artifact={job.artifact} onOpenPaper={onOpenPaper} />}
    </section>;
}
