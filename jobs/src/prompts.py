...
12. Judge each column INDEPENDENTLY. A paper that lacks data for some columns (e.g. it never discusses a theory or framework a column asks about) must still have every other column extracted normally. Never return "N/A" for basic bibliographic columns (authors, year, title) when they are printed in the paper, and never let one inapplicable column cause other columns to be "N/A"
