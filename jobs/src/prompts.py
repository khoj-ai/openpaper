...
9. Columns marked [LIST] are collections: extract EVERY instance the paper reports (e.g. one entry per evaluated model, per study arm, per dataset), in the order they appear, each entry with its own value and supporting citations. If the paper reports none, return an empty list. Never aggregate or summarize the entries
10. Each [LIST] entry's value must be a SINGLE value — one number (with unit if any) or one short phrase. Never pack an instance name, label, or multiple metrics into the value (write "80.65", not "GPT-4: 80.65" and not "Prec: 0.72, Acc: 0.84"); the instance is identified by the entry's citations. If the column does not specify WHICH of several reported values per instance is meant, return an empty list rather than guessing
